...
static bool sortbysec_desc(const std::pair<int, int>& a, const std::pair<int, int>& b) { return (b.second < a.second); }
                     ...
                     int popup_max_height_in_items /*= -1*/)
{
    ...
    if (*current_item >= 0 && *current_item < number_of_items)
    ...
    int show_count = number_of_items;
    ...
    {
        int items = popup_max_height_in_items + 3; // extra for search bar
    ...
    const ImVec2 search_icon_pos(ImGui::GetItemRectMax().x - label_size.x - style.ItemInnerSpacing.x * 2,
    ...
    if (move_delta != 0)
    {
        ...
        {
            focus_idx = ImClamp(focus_idx + move_delta, 0, number_of_items - 1);
        }
    }
    ...
    float height_in_items_f =
        (popup_max_height_in_items < 0 ? ImMin(number_of_items, 7) : popup_max_height_in_items) + 0.25f;
    ImVec2 size;
    size.x = 0.0f;
